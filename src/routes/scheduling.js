...
router.post('/book', async (req, res) => {
  try {
      ...
      try {
        ...
        if (err instanceof reservas.BookingAuthError) {
          return res.status(503).json({
            ...
            retry: true,
            message: 'No podemos conectar con el sistema de reservas. Escribenos por WhatsApp al 688 60 67 54.',
          });
        }
      }
    }
    ...
    if (!bookingClassId) {
      return res.status(400).json({ error: 'No hemos encontrado esa clase. Escribenos por WhatsApp al 688 60 67 54 y te la reservamos.' });
    }
